...
- **格式解锁**：把网易云 `.ncm`、QQ音乐 `.mflac`/`.mgg`/`.qmc*` 等本地加密音频解锁为标准格式
  - 纯 Python 实现，零新增依赖；源文件只读不动
  - NCM 自动提取标签（歌名/歌手/专辑）与封面并内嵌输出
  - 输出格式可选：自动（保留内层格式，推荐）/ 强制 FLAC / 强制 WAV；有损内层转无损容器不会提升音质，界面有明确提示
  - **QQ音乐新版 musicex 文件支持密钥导入**：一键从运行中的 QQ 音乐客户端读取登录态、按歌换取解密密钥并缓存本机（`~/.soniccheck/qqmusic_ekeys.json`，不过期、不上传），导入后即可离线解锁；解锁窗口内置图文教程入口
  - **QQ音乐解锁自动补齐标签**：musicex 文件解锁后按 song_mid 经官方详情接口取回歌名/歌手/专辑并内嵌 500×500 封面，文件名自动清理 `_EM`/`_EG` 等音质尾缀；可一键关闭纯离线解锁，失败自动降级为无标签不影响解锁
  - 注意：QQ音乐 STag 文件密钥不在文件内且无取回通道，无法离线解锁（可用 ≤19.43 旧版客户端重新下载）；请仅转换你合法获得的文件
